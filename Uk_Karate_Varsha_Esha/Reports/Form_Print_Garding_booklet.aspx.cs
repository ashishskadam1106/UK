...
public partial class Reports_Form_Print_Garding_booklet : System.Web.UI.Page
{
    ...
    private void Fill_Report()
    {
        ...
        ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Report_PrintGradingBooklet.rdlc");
        Ds_BookletPrint dsIncidents = GetData();
        ReportDataSource datasource = new ReportDataSource("Ds_BookletPrint", dsIncidents.Tables[0]);
        ReportViewer1.LocalReport.DataSources.Clear();
        ReportViewer1.LocalReport.DataSources.Add(datasource);
    }

    private Ds_BookletPrint GetData()
    {
        cmd = new SqlCommand("USP_GetStudentListForBookletPrint");
        using (con = new SqlConnection(str))
        {
            using (da = new SqlDataAdapter(cmd))
            {
                ...
                using (Ds_BookletPrint Receipt = new Ds_BookletPrint())
                {
                    da.Fill(Receipt, "USP_GetStudentListForBookletPrint"); //Name of dataTable
                    return Receipt;
                }
            }
        }
