...
public partial class Reports_Form_PrintStarterpackfrontpage : System.Web.UI.Page
{
    ...
    private void Fill_Report()
    {
        ...
        ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Report_PrintStarterpackfrontpage.rdlc");
        DS_Student dsIncidents = GetData();
        ReportDataSource datasource = new ReportDataSource("DS_Student", dsIncidents.Tables[0]);
        ReportViewer1.LocalReport.DataSources.Clear();
        ReportViewer1.LocalReport.DataSources.Add(datasource);
    }

    private DS_Student GetData()
    {
        cmd = new SqlCommand("USP_StudentClassInformationForPrint");
        using (con = new SqlConnection(str))
        {
            using (da = new SqlDataAdapter(cmd))
            {
                ...
                using (DS_Student Receipt = new DS_Student())
                {
                    da.Fill(Receipt, "USP_StudentClassInformationForPrint"); //Name of dataTable
                    return Receipt;
                }
            }
        }
