...
public partial class Reports_Form_PrintOverduePaymentsOfStudents : System.Web.UI.Page
{
    ...
    private void Fill_Report()
    {
        ...
        ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Report_PrintOverduePaymentsOfStudents.rdlc");
        DS_OverduePayments dsIncidents = GetData();
        ReportDataSource datasource = new ReportDataSource("DS_OverduePayments", dsIncidents.Tables[0]); //name of dataset

        ReportViewer1.LocalReport.DataSources.Clear();
        ReportViewer1.LocalReport.DataSources.Add(datasource);
    }

    private DS_OverduePayments GetData()
    {
        cmd = new SqlCommand("USP_Get_OverduePaymentofStudents");
        using (con = new SqlConnection(str))
        {
            using (da = new SqlDataAdapter(cmd))
            {
                ...
                using (DS_OverduePayments Bill = new DS_OverduePayments())
                {
                    da.Fill(Bill, "USP_Get_OverduePaymentofStudents"); //Name of dataTable
                    return Bill;
                }
            }
        }
